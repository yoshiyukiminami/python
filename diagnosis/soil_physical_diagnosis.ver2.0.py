...
# Ver1.2-s・・圃場位置情報で特殊な測定（例：Bのみ5ヵ所測定など）をした場合、圃場位置情報エラーをスキップする
# ver2.0・・ver1.2-sをベースに、グラフタイトルからIDを削除（タイトル名が長すぎる）、時期と測定日の順番を入れ替え
# グラフの保存名のバージョン表記をver1.2からver2.0に変更
...
def graphset_1x3(df_all, df_ave, line_color, line_shape, y, graph_title, save_pic_name2):
    ...
    filename = filedir + save_pic_name2 + '_ver2.0.jpeg'
    ...
    if not item_list_count == 1:
        ...
        if not jiki_list_count == 1:
            ...
            if not sokuteibi_count == 1:
                ...
                graph_titles = df_all[['hojyomei', 'item', 'jiki', 'ymd']].values
